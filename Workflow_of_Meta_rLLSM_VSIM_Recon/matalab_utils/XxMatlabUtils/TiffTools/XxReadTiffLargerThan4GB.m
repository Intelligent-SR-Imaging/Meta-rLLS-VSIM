...
if isempty(img_amount)+isempty(slcies_amount)+isempty(frames_amount)==3 %表示只有一张图
    img_amount_read=1;
else
    img_amount_read=img_amount;
end
file_handle=fopen(TiffPath,'r');
fseek(file_handle, pic_info.StripOffsets, 'bof');%略过开头的字符
