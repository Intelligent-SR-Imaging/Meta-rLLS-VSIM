"""
use pretrained RL-DFN to reconstruct a whole 3D Image
...
import torch
from models.Dual_Cycle_section_rldeconv_test import DualGANASectionModel
from utils.read_mrc import read_mrc, write_mrc
from utils.utils import Align,padding
import numpy as np
import glob
import argparse
...
def parse_option():
    ...
    parser.add_argument('--Data_path', type=str, default="/Code_for_3D_IsoRecon/data/TestData", help='path to dataset')
    # path to save
    parser.add_argument('--Save_path', type=str, default="/Code_for_3D_IsoRecon/result", help='path to dataset')
    # path to pretrained model
    parser.add_argument('--model_path', default="/Code_for_3D_IsoRecon/train_RL_DFN/test/checkpoints/iteration17600", help='checkpoint want to load')
    # The model's name
    parser.add_argument('--name', type=str, default="F-actin")
    parser.add_argument('--Struct', type=str, default="F-actin")
...
if __name__ == "__main__":
    ...
    #test data prepare
    args = parse_option()
    ViewA_root = Source_path + args.Data_path+"/ViewA/*.tif"
    ViewB_root = Source_path + args.Data_path + "/ViewB/*.tif"
    save_path = Source_path + args.Save_path+"/"
    rot_view_1 = glob.glob(ViewA_root)
    rot_view_2 = glob.glob(ViewB_root)
    unrot_view = glob.glob(ViewB_root)
    ...
    model_type = "Unet"
    psf_know = True
    head,_ = read_mrc(Source_path+args.mrc_root)
    ...
    model = DualGANASectionModel(patch_x=patch_x_size, patch_y=patch_y_size, patch_z=patch_z_size, name=name,
                                 ...
                                 psfA_root=Source_path+args.psfA_root, psfB_root=Source_path+args.psfB_root,
    ...
    for index in tqdm.tqdm(range(file_len)):
       ...
       # fake = fake.astype(np.uint16)
        fake_root = Source_path+args.Save_path+"/" + str(index+1).zfill(3) + ".tif"
