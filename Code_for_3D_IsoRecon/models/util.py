...
def normalize(img_np, data_type = float):
    """normalize a Tensor array.
            Args:
                input_image (tensor) :  the input image tensor array
                data_type (type) :  the desired type of the converted numpy array
    """
    img_min = np.min(img_np)
    img_max = np.max(img_np)
...
def noisy(noise_typ, image, sigma=0.1, peak=0.1, is_tensor=False, is_normalize=True):
    """Add noise on a Tensor array.
            Args:
                noise_typ (str) : the type noise to add
                image (tensor) :  the input image tensor array
                sigma,sigma (float) :  the setting of noise
    """
    if is_tensor:
...
def get_mse(source, target):
    """Calculate MSE.
        Args:
            source, target (tensor) :  the input image tensor array
...
def get_snr(img_original, img_noised):
    """Calculate MSE.
        Args:
            img_original, img_noised(tensor) :  the input image tensor array
    """
...
def get_psnr(source, target, data_range):
    """Calculate PANR.
        Args:
            source, target(tensor) :  the input image tensor array
            data_range(float) : the range of pixel value
    """
