...
class DualGANASectionModel(BaseModel):
    """ RL-DFN Model, train based on cGAN
        Args:
            ...
            TV_loss_weight (float): weight of TV Loss
    ...
    def __init__(self, patch_x=128, patch_y=128, patch_z=64, gpu_ids=[3], gan_mode='vanilla', name="experiment name",
                 ...
                 lr=0.0001, beta1=0.9,  batch_size=1, TV_loss_weight=0.0001, cycle_loss_weight=5000):

        self.patch_x = patch_x
        self.patch_y = patch_y
        self.patch_z = patch_z
        self.TV_loss_weight = TV_loss_weight
        self.cycle_loss_weight = cycle_loss_weight*100
        ...
        self.RL_deconv = RL_Deconv(psfA_root=psfA_root, psfB_root=psfB_root, X_dim=patch_x, Y_dim=patch_y,
                                   Z_dim=patch_z,
                                   device=device, batch_size=batch_size)

        self.RL_deconv.set_device(device)
    ...
    def section_prepare(self,H = None,W = None,head = None,pixel_off = 0,theta_off = 0,theta = 30.8,MIP = False):
        ...
        cor = section_position(H, W, axis="Y", theta=pi * ((180 - theta) / 180)) + np.array([pixel_off, 0, 0]).reshape(1,
                                                                                                                      -1)
    ...
    def forward(self):
        ...
        for i in range(self.deconv_iter):
            cur1, cur2 = self.RL_deconv(real_rot1, real_rot2, fusion)
        ...
        self.fake = fusion
        self.degeration_A, self.degeration_B = self.RL_deconv.degradation(self.fake)
    ...
    def visualization(self,root):
        ...
        fake_section_rootA = root + "/total_iets" + str(self.total_iters) + "fake_sectionA.mrc"
        fake_section_rootB = root + "/total_iets" + str(self.total_iters) + "fake_sectionB.mrc"
        real_section_rootA = root + "/total_iets" + str(self.total_iters) + "real_sectionA.mrc"
        real_section_rootB = root + "/total_iets" + str(self.total_iters) + "real_sectionB.mrc"
        ...
        if self.total_iters%self.save_freq == 0:
            ...
            fake_section_A = self.section_dict["fake0"]
            fake_section_B = self.section_dict["fake1"]
            real_section_A = self.section_dict["real0"]
            real_section_B = self.section_dict["real1"]
            ...
            self.head[0][0] = self.H
            self.head[0][1] = self.W
            self.head[0][2] = 1
            self.head[0][3] = 2

            write_mrc(fake_section_rootA, fake_section_A, self.head)
            write_mrc(fake_section_rootB, fake_section_B, self.head)
            write_mrc(real_section_rootA, real_section_A, self.head)
            write_mrc(real_section_rootB, real_section_B, self.head)
    ...
    def backward_D_slice(self, netD, real, fake, slice_axis_real, slice_axis_fake):
        ...
        pred_real,section_real = self.iter_f_section(real, netD, slice_axis_real,real=True)

        pred_fake,section_fake = self.iter_f_section(fake.detach(), netD, slice_axis_fake,real=True)

        self.section_dict["real" + str(slice_axis_real)] = section_real[:, :, 0]
        self.section_dict[("fake" + str(slice_axis_fake)).replace("3", "0").replace("2", "1")] = section_fake[:, :, 0]
        # real
    ...
    def backward_G(self):
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + (self.loss_cycle1+self.loss_cycle2)*self.cycle_loss_weight+self.TV_loss_weight*self.TV_loss(self.fake)
    ...
    def iter_f_section(self, input, function, slice_axis,real = False):
        if slice_axis!=6:
            ...
            if slice_axis == 2:
                ...
                MIP = input[:,:,:,4:60].max(dim = -1)
                H_MIP = input.shape(1)-self.H
                W_MIP = input.shape(2)-self.W
                x = random.randint(0,H_MIP)
                y = random.randint(0, W_MIP)
                section = MIP[:,x:x+self.H,y:y+self.W].reshape(-1, 1, self.H,self.W).permute(0, 1, 3, 2)

            elif slice_axis == 7:
                y = random.randint(20, self.patch_x-20)
                MIP = input[:,y-15:y+15,:,:].max(dim = 1)
                W_MIP = input.shape(1)-self.W
                H_MIP = input.shape(2)-self.H
                x = random.randint(0,H_MIP)
                y = random.randint(0, W_MIP)
                section = MIP[:,x:x+self.H,y:y+self.W].reshape(-1, 1, self.H,self.W).permute(0, 1, 3, 2)


            section_ = section.cpu().detach().numpy().reshape(-1, self.W, self.H).transpose(2, 1, 0).astype(np.float32)
            output_slice = function(section)
            if real:
                return output_slice, section_
            else:
                return output_slice
    ...
    def load_checkpoint(self,dataroot,map_location=None):
        if self.deconv_iter==1:
            self.netF_A_list[0].load_state_dict(
                torch.load(dataroot + 'net_FA.pth', map_location=map_location, weights_only=False))
            self.netF_B_list[0].load_state_dict(
                torch.load(dataroot + 'net_FB.pth', map_location=map_location, weights_only=False))
            self.netG_list[0].load_state_dict(
                torch.load(dataroot + 'net_G.pth', map_location=map_location, weights_only=False))
            self.netD_A.load_state_dict(
                torch.load(dataroot + 'netD_A.pth', map_location=map_location, weights_only=False))
            self.netD_B.load_state_dict(
                torch.load(dataroot + 'netD_B.pth', map_location=map_location, weights_only=False))
        elif  self.deconv_iter == 2:
