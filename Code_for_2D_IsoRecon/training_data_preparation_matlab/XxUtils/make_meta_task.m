function make_meta_task(LR_path, SR_path, save_path, SegX, SegY, SegZ, num_seg, st_selected)
...
if nargin < 8, st_selected = [2, 5]; end
...
for i=3:length(stru_gt)
    ...
    for t = 1:1:length(tifList_lr)
        ...
        for j=3:length(cell_gt)
            ...
            elseif ismember(t, st_selected)   % training set
                % segmentation
                [data_seg,gt_seg] = XxDataSeg_ForTrain_Lattice_3D(data_lr, data_gt, num_seg, SegX, SegY, SegZ);
                for l=1:size(data_seg,4)
                    % size of LLSM (7,h,w)
