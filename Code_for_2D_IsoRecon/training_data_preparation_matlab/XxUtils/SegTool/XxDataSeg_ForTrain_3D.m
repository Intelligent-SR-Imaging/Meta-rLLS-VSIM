function [gt_seg,data_seg,rl_seg] = XxDataSeg_ForTrain_3D(gt, data, num_seg, Nx, Ny, RotFlag, depth, half_side_flag)
...
for i = 1:num_seg
    ...
    if RotFlag >= 1 % if random rotate
        degree = randi(360, 1);
        patch_gt = imrotate(sum(gt(2*x1-1:2*x2,2*y1-1:2*y2,:), 3),degree,'bilinear','crop');
        tx_gt = new_r-Nx;
        ty_gt = new_l-Ny;
        patch_gt = patch_gt(tx_gt+1:tx_gt+2*Nx,ty_gt+1:ty_gt+2*Ny);
        ...
        patch_gt = sum(gt(2*x1-1:2*x2,2*y1-1:2*y2,:), 3);
        active_range_gt = double(prctile(patch_gt(:),99.9)) / double(prctile(patch_gt(:),0.1)+1);
...
%         subplot(1,2,2), imagesc(patch_gt(:,:,1)); axis image, axis off;
    ...
    if RotFlag >= 1
        tgt = imrotate(gt(2*x1-1:2*x2,2*y1-1:2*y2,:),degree,'bilinear','crop');
        tdata = imrotate(data(x1:x2,y1:y2,:),degree,'bilinear','crop');
        if isempty(gt_seg)
            gt_seg = tgt(tx_gt+1:tx_gt+2*Nx,ty_gt+1:ty_gt+2*Ny,:);
        else
            h_gt = size(tgt, 3);
            gt_seg(:,:,end+1:end+h_gt) = tgt(tx_gt+1:tx_gt+2*Nx,ty_gt+1:ty_gt+2*Ny,:);
        end
        if isempty(data_seg)
            if half_side_flag
        ...
        if isempty(gt_seg)
            gt_seg = gt(2*x1-1:2*x2,2*y1-1:2*y2,:);
        else
            h_gt = size(gt, 3);
            gt_seg(:,:,end+1:end+h_gt) = gt(2*x1-1:2*x2,2*y1-1:2*y2,:);
        end
        if isempty(rl_seg)
