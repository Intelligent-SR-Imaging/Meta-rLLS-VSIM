function [data_seg_l,data_seg_h,gt_seg] = XxDataSeg_ForTrain_3D_N2N(gt, data_l, data_h, num_seg, Nx, Ny, RotFlag, depth, half_side_flag)
...
thresh_mask = 10e-2;
ksize = 3;
mask = XxCalMask(data_h(:,:,ceil(depth/2)),ksize,thresh_mask);
while sum(mask(:)) < 1e3
...
for i = 1:num_seg
    ...
    if RotFlag >= 1 % if random rotate
        ...
        patch_gt = imrotate(sum(gt(sr_ratio*x1-1:sr_ratio*x2,sr_ratio*y1-1:sr_ratio*y2,:),3),degree,'bilinear','crop');
        ...
        patch_gt = sum(gt(sr_ratio*x1-1:sr_ratio*x2,sr_ratio*y1-1:sr_ratio*y2,:),3);
        sum_patch_gt = sum(patch_gt(:));
    end
