function [data_seg_l,data_seg_h,gt_seg] = XxDataSeg_ForTrain_N2N(data_l, data_h, gt, num_seg, Nx, Ny, RotFlag)
% also suitable for R2R, where data_l=data, data_h=data_gt, gt=data_comp,
% because the former two definitely share the same size
...
for i = 1:num_seg
    %pick patch points according to the mask
    p = randi(l_list,1);
